...
public function users_log(){
	...
	$this->load->view('header');
	$this->load->view('menu');
	$this->load->view('general/users_log');
	$this->load->view('footer');
...
public function bill_summary_detail($id)
	...
	public function insert_sub_menu(){
		...
		public function delete_ipc_files(){
...
public function bill_summary_detail_amount($id){
	extract($_POST);
	$this->load->view('header');
	$this->load->view('menu');
	$data['id']=$id;
	$this->load->view('ppms/bill_summary_detail_amount',$data);
	$this->load->view('footer');

}

public function boq_amount_percent($id){
	extract($_POST);
	$this->load->view('header');
	$this->load->view('menu');
	$data['id']=$id;
	$this->load->view('ppms/boq_amount_percent',$data);
	$this->load->view('footer');
...
public function get_bill_summary(){
	...
	$data["sub_sub_project_id"]=$id;
	$this->load->view("ppms/get_bill_summary",$data);
...
public function add_amount_billsummary_cells(){
    ...
    $done=$this->db->query("insert into bill_summary_amount set grp_id=$newVAL,billsummary_id=$lopoo1->billsummary_id,amount=$lopoo1->billsummary_amt,subproject_id=$projectID,sub_sub_project_id=$SubProjectID");
	}
	if($done){
		echo 1;
	}else{
...
/////////////////////////////////////////////////////////////////
public function enable_inputs(){
extract($_POST);
	$result1 = $this->db->query("SELECT * FROM ppms_billsummary 
	where subproject_id=$subID")->result();
	echo $this->db->last_query();
	echo "<br>";
	foreach($result1 as $result2){
		$doneeee="select * from ppms_ipc_billsummary where billsummary_id=$result2->billsummary_id and ipc_id=$ipc and subproject_id=$subID and sub_subproject_id=$result2->sub_sub_project_id";
 $doneAL=$this->db->query($doneeee)->row();
 if($doneAL){

 }else{

 echo $doneeee="insert into ppms_ipc_billsummary set billsummary_id=$result2->billsummary_id,ipc_id=$ipc,ipc_bs_amount=0,subproject_id=$subID,sub_subproject_id=$result2->sub_sub_project_id";
...
/////////////////////////////////////////////////////////
	$ipac_amount4201 = $this->input->post('ipac_amount420');
	...
	//$done=$this->db->query("update ppms_subproject_detail set city_id=$city_id,start_date='$start_date',end_date='$end_date',subproject_name='$sub_project_name',subproject_percentage='$percentage',subproject_amount='$project_amount' where subproject_name='$sub_project_name'");	
		//}else{
			$ipc_amount=str_replace(',', '', $ipc_amount);
			$ipac_amount420=str_replace(',', '', $ipac_amount4201);
		...
		ipac_submitted_date='$submitted_date',ipc_no='$ipc_no',certificate_type='$certificate_type',already_ipc='$already_ipc'");	
		//}
		...
		*/
		...
		if ($this->upload->do_upload("attach_file")) {
		  ...
		  $count = count($_FILES['files']['name']);
		   ...
		   $SubprojectsID=$this->db->query("select subproject_name,project_id from ppms_subproject where subproject_id=$sub_project_id")->row();

		   $subProjectName=$SubprojectsID->subproject_name;

		   $projectsID=$this->db->query("select project_name from ppms_project where project_id=$SubprojectsID->project_id")->row();
		   $projectName=$projectsID->project_name;

		   $doneee=$this->db->query("select * from sms_group where organization_id=$orgid")->row();
		   ...
		   //echo 92.$mobileNumber;
						   ...
						   $complaint_user_msg = "Dear ".$sms->designation_name. " " .$projectName ."-".$subProjectName. " Has Been Generated at:  ".$sms->organization_name." On Date :".$date;
						   //echo $date;exit;
...
public function mobilization_advance($id){

	extract($_POST);
	$this->load->view('header');
	$this->load->view('menu');
	$data["id"]=$id;
	$this->load->view('ppms/mobilization_advance',$data);
	$this->load->view('footer');
...
public function ipc_pmu_payment(){
extract($_POST);
$payment_date=date('Y-m-d');
$done=$this->db->query("select * from ppms_gross_payment where ctp_id=$cptid and ipc_no=$ipcno and ipc_id=$ipcid and verified=1 add_less='$add_less'")->row();
if($done){

$done=$this->db->query("update ppms_gross_payment set ppms_payment_date='$payment_date',
tax_value=$tax,gross_payment=$gpayment,remarks='$remarks',add_less='$add_less',verified=1 where ctp_id=$cptid and ipc_no=$ipcno and ipc_id=$ipcid");

}else{
//$done=$this->db->query("insert into ppms_net_payment set ppms_payment_date='$payment_date',ipc_no=$ipcno,ipc_id=$ipcid");
//$lastid = $this->db->insert_id();
$done=$this->db->query("insert into ppms_gross_payment set ppms_payment_date='$payment_date',verified=1,
ctp_id=$cptid,ipc_no=$ipcno,ipc_id=$ipcid,tax_value=$tax,gross_payment=$gpayment,remarks='$remarks',add_less='$add_less'");
}
}


public function ppms_gross_payment(){
extract($_POST);
$payment_date=date('Y-m-d');
$tax_amount=str_replace(',', '', $tax_amount);
if($organization_id==3){
	$done=$this->db->query("update ppms_gross_payment set ppms_payment_date='$payment_date',ctp_id=$tax_id,
	tax_value=$tax,gross_payment=$tax_amount,remarks='$remarks',add_less='$add_less',verified=$vid where ppms_payment_id=$id");
	...
	if($vid==1){
	$done=$this->db->query("update ppms_gross_payment set ppms_payment_date='$payment_date',ctp_id=$tax_id,
	tax_value=$tax,gross_payment=$tax_amount,remarks='$remarks',add_less='$add_less',verified=1 where ppms_payment_id=$id");
	if($done){
...
public function ipc_pmu_payment_pmu(){
	extract($_POST);
	$payment_date=date('Y-m-d');
	$done=$this->db->query("select * from ppms_gross_payment where ctp_id=$cptid and ipc_no=$ipcno and ipc_id=$ipcid and verified=2 and add_less='$add_less'")->row();
	if($done){
	
	$done=$this->db->query("update ppms_gross_payment set ppms_payment_date='$payment_date',
	tax_value=$tax,gross_payment=$gpayment,remarks='$remarks',add_less='$add_less',verified=2 where ctp_id=$cptid and ipc_no=$ipcno and ipc_id=$ipcid");
	
	}else{
	//$done=$this->db->query("insert into ppms_net_payment set ppms_payment_date='$payment_date',ipc_no=$ipcno,ipc_id=$ipcid");
	//$lastid = $this->db->insert_id();
	$done=$this->db->query("insert into ppms_gross_payment set ppms_payment_date='$payment_date',verified=2,
	ctp_id=$cptid,ipc_no=$ipcno,ipc_id=$ipcid,tax_value=$tax,gross_payment=$gpayment,remarks='$remarks',add_less='$add_less'");
	}
	}
...
public function ipc_amt_engineer(){

	
	extract($_POST);
	$ipc_amt_engineer=str_replace(',', '', $ipc_amt_engineer);
	$ipc_amt_orginal=str_replace(',', '', $ipc_amt_orginal);
	$run = $this->db->query( "update ppms_ipac set ipac_amount=$ipc_amt_engineer,ipa_amount=$ipc_amt_orginal where ipac_id=$ipcid");
		if($run){
...
public function ipac_dashboard()
{
	extract($_POST);
	$this->load->view('header');
	$this->load->view('menu');
	$this->load->view('ppms/dashboard_ipc');
	$this->load->view('footer');
}
...
/////////////////////////////////
public function ipc_pmu_ciu_ipc_tax(){
	extract($_POST);
	$payment_date=date('Y-m-d');
	///$tax_amount=str_replace(',', '', $tax_amount);
	$done=$this->db->query("select * from ppms_gross_payment where ctp_id=$tax_id and ipc_no='$ipcno' and ipc_id=$ipc_id and verified=$verified and add_less='$add_less'")->row();
	if($done){
	
	$done=$this->db->query("update ppms_gross_payment set ppms_payment_date='$payment_date',
	tax_value=$tax,gross_payment=$tax_amount,remarks='$remarks',add_less='$add_less',verified=$verified where ctp_id=$tax_id and ipc_no='$ipcno' and ipc_id=$ipcid");
	
	}else{
	//$done=$this->db->query("insert into ppms_net_payment set ppms_payment_date='$payment_date',ipc_no=$ipcno,ipc_id=$ipcid");
	//$lastid = $this->db->insert_id();
	$done=$this->db->query("insert into ppms_gross_payment set ppms_payment_date='$payment_date',verified=$verified,
	ctp_id=$tax_id,ipc_no=$ipcno,ipc_id=$ipc_id,tax_value=$tax,gross_payment=$tax_amount,remarks='$remarks',add_less='$add_less'");
	}
if($done){
	redirect(base_url('Certificate_tax/apply_taxes_pmu_ciu/').$ipcno);
}
	}
/////////////////////////////////////////////////////
...
extract($_POST);
 $doneCheck="select * from ppms_ipc_billsummary where billsummary_id=$sid and ipc_id=$ipcid";
$doneCheck1=$this->db->query($doneCheck)->row();
if($doneCheck1){
	 $doneeee="update ppms_ipc_billsummary set ipc_bs_amount=$amt where billsummary_id=$sid and ipc_id=$ipcid";
	$done=$this->db->query($doneeee);
}else{
	echo $doneeee="insert into ppms_ipc_billsummary set billsummary_id=$sid,ipc_id=$ipcid,ipc_bs_amount=$amt,subproject_id=$sprojectID,sub_subproject_id=$subprojectID";
	$done=$this->db->query($doneeee);
}
