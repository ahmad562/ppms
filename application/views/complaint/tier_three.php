 ...
 $i++;
  ...
  <?php 
    $result = $this->db->query("SELECT complaint_date FROM gre_complaint_detail where tier_id=3")->row();
    $result1 = $this->db->query("SELECT tier_days FROM grm_tier_tbl where tier_id=3")->row();
  $Date2 = date('Y-m-d', strtotime($result->complaint_date . " + $result1->tier_days day"));
  $Date3 = date('Y-m-d', strtotime($Date2 . " - 30 day"));
...
// if($woweekends<=0){
//     $update = $this->db->query("update gre_complaint_detail set tier_id=0 where tier_id=3 AND complaint_date='$Date3'");
// }
     ?>
