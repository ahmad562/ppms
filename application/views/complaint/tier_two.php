 ...
 $i++;
  ...
  <?php 
    $result = $this->db->query("SELECT complaint_date FROM gre_complaint_detail where tier_id=2")->row();
    $result1 = $this->db->query("SELECT tier_days FROM grm_tier_tbl where tier_id=2")->row();
  $Date2 = date('Y-m-d', strtotime($result->complaint_date . " + $result1->tier_days day"));
  $Date3 = date('Y-m-d', strtotime($Date2 . " - 20 day"));
