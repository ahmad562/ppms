...
$ipc_iddd=$id;
////////////////////////////////////////////////////// 
 $cid = $this->db->query("SELECT emp_id,emp_name,organization_id FROM ppms_subproject_assign AS psa,emp AS e
...
?></td>
    ...
    SELECT MAX(ipac_forward_id) AS MAX FROM ppms_ipac_forward
    WHERE `ipac_id`=$item->ipac_id
    )")->row();
//echo $this->db->last_query();
//echo $orgName->organization_id;
//echo "<hr>";
//echo $checkss->organization_id;

if($orgName->organization_id==$checkss->organization_id){
/////////////////////////////////////////////////////////
...
$sqlORG1 = $this->db->query($sqlORG)->row();
 ...
 $sqlChecked = "SELECT COUNT(*) AS selected_items FROM items_ipc WHERE item_id IN (SELECT item_id FROM items WHERE organization_id = $orgName->organization_id) and status_id=1 and ipc_id=$item->ipac_id";
$sqlChecked1 = $this->db->query($sqlChecked)->row();


if ($sqlORG1->total_items == $sqlChecked1->selected_items) {
if(($checkss->organization_id==3) and ($checkss->status_id==4) and ($checkss->flag_id==1)){
...
if($sqlPayment1){
   echo "<label class='label label-success'>
    <font color='white'>IPC Paid</font></lable>";
}else{
?>
...
?>
                    ...
                    echo "<label class='label label-danger'>File Moved</label>";
...
?>
                    ...
                    WHERE `subproject_id`=$subID AND `sub_sub_project_id`=$main->sub_sub_project_id AND billsummary_id=$item->billsummary_id
                                                            ...
                                                            if($newVAL >=1){
                                                            echo number_format($newVAL=$maxiii->amount,2);
