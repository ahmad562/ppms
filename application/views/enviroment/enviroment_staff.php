...
?>
 ...
 $i++;
	...
	if(isset($_POST['add'])){
		extract($_POST);
$done=$this->db->query("select * from ppms_es_staff where es_staff_name='$staff_name'")->row();
	if($done){

	}else{
	$done=$this->db->query("insert into ppms_es_staff set es_staff_name='$staff_name',designation_id=$designation_id,sps_id=$site_id,es_staff_status=$status_id");	
	}
