...
<div class="col-md-6 col-xl-2">
 ...
 SELECT COUNT(*) AS tot_paid 
FROM 
ppms_ipac AS ipc,
ppms_subproject AS ppss,
ppms_ipac_forward AS ppif
WHERE ipc.ipac_id=ppif.ipac_id
AND ipc.subproject_id=ppss.subproject_id

AND ppif.ipac_forward_id=
(
SELECT MAX(ipac_forward_id) FROM ppms_ipac_forward AS pif1
WHERE pif1.ipac_id=ppif.ipac_id
AND pif1.flag_id=1
AND pif1.`status_id`=4
)
")->row();
                                            }else{
                                             if($desig_emp->designation_id==8){
                                             ...
                                             $tot_ipace = $this->db->query("SELECT COUNT(*) AS tot_paid 
                                             FROM 
                                             ppms_ipac AS ipc,
                                             ppms_subproject AS ppss,
                                             ppms_ipac_forward AS ppif
                                ...
                                <div class="card-block">
                                    ...
                                    <ul>
                                        ...
                                        <?php 
                                            ...
                                            if($groupID == 5 or $groupID == 6 or $groupID == 1 or $groupID == 2){
                                             $tot_ipaceR = $this->db->query("
 SELECT COUNT(*) AS tot_rturn 
FROM 
ppms_ipac AS ipc,
ppms_subproject AS ppss,
ppms_ipac_forward AS ppif
...
WHERE pif1.ipac_id=ppif.ipac_id
AND pif1.flag_id=1
AND pif1.`status_id`=3
)
")->row();
                                            }else{
                                             if($desig_emp->designation_id==8){
                                                //echo "pakistan";
                                                 $tot_ipaceR = $this->db->query("SELECT count(*) as tot_rturn FROM 
                                               ppms_ipac as ppi,ppms_subproject as ppsp,ppms_project as ppp,assign_regional AS ar
                                               where ppi.subproject_id=ppsp.subproject_id
                                               and ppi.subproject_id=ar.subproject_id
                                               and ppsp.project_id=ppp.project_id
                                             ...
                                             $tot_ipaceR = $this->db->query("SELECT COUNT(*) AS tot_rturn 
                                             FROM 
                                             ppms_ipac AS ipc,
                                             ppms_subproject AS ppss,
                                             ppms_ipac_forward AS ppif
